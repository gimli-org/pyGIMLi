...
There is also a complex dense matrix `pg.matrix.CMatrix`.
...
There is complex-valued variant `pg.matrix.CSparseMapMatrix`.
...
First, there is an identity matrix `IdentityMatrix`. No elements are
...
* laterally, spatially or temporally constrained inversion: regularization between model cells of each frame but also between the frames Note that the matrices only have to be defined once and can appear multiply.
...
`RepeatHMatrix`, `RepeatVMatrix`, `RepeatDMatrix` hold a single matrix that
is repeated horizontally, vertically or diagonally.
`NDMatrix`,
...
decomposition is done and the eigenvalues $D$ and -vectors $Q$ are stored
so that the operator works as

$$ C^{-0.5}\cdot x=Q\cdot D^{-0.5}\cdot Q^T \cdot x $$

```{code-cell} ipython3
grid = pg.createGrid(5, 4)
C = pg.matrix.GeostatisticConstraintsMatrix(grid, I=[3, 1], dip=-30)
a = np.zeros(grid.cellCount())
a[5] = 1.0
pg.show(grid, C.mult(a), label="Constraint Weights",
        cMin=-0.5, cMax=0.5, cMap="RdBu_r")
pg.wait()
```
