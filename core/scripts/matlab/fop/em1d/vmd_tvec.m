...
% (rmin,rmax),(t) fuer Tiefe z nach dem Abschalten des
...
%
% Output:
% HZP,HZ,HR,EPHI(NT,NR) fuer [RMIN,RMAX] in Ebene z = const
% RR:    Abstaende
...
% Abstaende und Zeiten
nr = 1;
nt=length(t);
% t = tmin*q.^((1:nt)-1);
% q=(t/min(t)).^(1./(0:nt-1)');
l=log10(max(t)/min(t))/(nt-1);
q = 10^(l);
...
omega = q.^(1-(-nc+nc0+(1:ncnt)))./min(t);

for nn = 1:ncnt,
    n = -nc+nc0+nn;
    % Rechnungen im Frequenzbereich:
    [fields,rr] = ...
        vmd_f(iout(2:4),omega(nn)/(2*pi),rho,d,z,0,r,nr,1);
